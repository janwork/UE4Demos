...
AGASCharacter::AGASCharacter()
{
	...
	AbilitySystemComponenet = CreateDefaultSubobject<UGASAbilitySystemComponent>("AbilitySystemComp");
	AbilitySystemComponenet->SetIsReplicated(true);
	AbilitySystemComponenet->SetReplicationMode(EGameplayEffectReplicationMode::Minimal);
...
{
	//加技能效果加入技能系统里
	if (AbilitySystemComponenet && DefaultAttributeEffect) {
		FGameplayEffectContextHandle EffectContext = AbilitySystemComponenet->MakeEffectContext();
		EffectContext.AddSourceObject(this);

		FGameplayEffectSpecHandle SpecHandle = AbilitySystemComponenet->MakeOutgoingSpec(DefaultAttributeEffect, 1, EffectContext);

		if (SpecHandle.IsValid())
		{
			FActiveGameplayEffectHandle GEHandle = AbilitySystemComponenet->ApplyGameplayEffectSpecToSelf(*SpecHandle.Data.Get());
		}
	}
}
...
{
	if (HasAuthority() && AbilitySystemComponenet) 
	{
		//获得技能
		for (TSubclassOf<UGASGameplayAbility>& StartupAbility : DefaultAbilities)
		{
			AbilitySystemComponenet->GiveAbility(
...
void AGASCharacter::OnRep_PlayerState()
{
	...
	if (AbilitySystemComponenet && InputComponent) {
		const FGameplayAbilityInputBinds Binds("Confirm", "Cancel", "EGASAblitiyInputID", static_cast<int32>(EGASAblitiyInputID::Confirm), static_cast<int32>(EGASAblitiyInputID::Cancel));
		AbilitySystemComponenet->BindAbilityActivationToInputComponent(InputComponent, Binds);
	}
}
...
void AGASCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (AbilitySystemComponenet && InputComponent) {
		AbilitySystemComponenet->BindAbilityActivationToInputComponent(InputComponent, FGameplayAbilityInputBinds(FString("Confirm"), FString("Cancel"), FString("EGASAblitiyInputID"), static_cast<int32>(EGASAblitiyInputID::Confirm), static_cast<int32>(EGASAblitiyInputID::Cancel)));
	}
}


void AGASCharacter::Punch() {
...
{
	// If GAS is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in GAS.Build.cs is not automatically propagated
